    ...
    <main className="h-[100svh] overflow-hidden bg-[#fffaf0] text-[#252422] selection:bg-[#eb5e28]/25">
      <nav className="absolute inset-x-0 top-0 z-50">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-5 sm:px-8 lg:px-10">
          ...
          >
            <span className="grid grid-cols-4 gap-1" aria-hidden="true">
              {Array.from({ length: 7 }, (_, index) => (
                <span
                  key={index}
                  className="h-1.5 w-1.5 rounded-[2px] bg-[#eb5e28] transition-transform group-hover:-translate-y-0.5 lg:bg-[#fffaf0]"
                />
              ))}
            </span>
          ...
          <Link
            href="/"
            className="group flex items-center gap-2 rounded-full border border-[#252422]/10 bg-white/60 px-4 py-2.5 text-xs font-bold backdrop-blur-md transition-colors hover:bg-white lg:border-white/20 lg:bg-white/10 lg:text-[#fffaf0] lg:hover:bg-white/15"
          >
        ...
        <section className="relative hidden h-full overflow-hidden bg-[#eb5e28] px-10 pb-8 pt-24 text-[#fffaf0] lg:flex lg:flex-col lg:justify-between xl:px-16">
          ...
          >
            ...
            <p className="mt-5 max-w-lg text-sm leading-6 text-white/75 xl:text-base">
              Your memories, milestones, and unfinished chapters are waiting
          ...
          >
            ...
            >
              {previewWeeks.map((week) => (
                <span
                  key={week}
                  className={`aspect-square rounded-[2px] ${week < 92 ? (week % 17 === 0 ? "bg-[#eb5e28]" : week % 29 === 0 ? "bg-[#f0c955]" : "bg-[#252422]") : "bg-[#ded8ce]"}`}
                />
              ))}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-white/60">
                  Return to notes, moods, and memories.
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-white/60">
                  Your personal story remains yours.
                </p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="rounded-[1.75rem] border border-[#252422]/10 bg-white/75 p-5 shadow-[0_24px_80px_rgba(37,36,34,0.10)] backdrop-blur-sm sm:p-7">
              <div className="mb-5">
                ...
                <p className="mt-3 text-sm leading-6 text-[#6d6861]">
                  Choose a secure way to return to your life map.
                </p>
              </div>
                ...
                {tab === "oauth" ? (
                  ...
                  >
                    {error && (
                      <div
                        role="alert"
                        className="rounded-2xl border border-red-500/20 bg-red-50 px-4 py-3 text-sm font-medium text-red-700"
                      >
                        {error}
                      </div>
                    )}
                  ...
                  >
                    ...
                    <label className="block">
                      <span className="mb-1.5 block text-[10px] font-bold uppercase tracking-[0.14em] text-[#625f59]">
                        Email address
                      </span>
                      <span className="relative block">
                        ...
                        <input
                          ...
                          className="h-12 w-full rounded-2xl border border-[#252422]/12 bg-[#f3ede2] pl-11 pr-4 text-sm text-[#252422] outline-none transition-all placeholder:text-[#9a9287] focus:border-[#eb5e28] focus:ring-4 focus:ring-[#eb5e28]/10"
                        />
                      </span>
                    </label>
                    <label className="block">
                    ...
                    <motion.button
                      ...
                      type="submit"
                      disabled={loading}
                      className="group flex min-h-12 w-full items-center justify-center gap-4 rounded-full bg-[#252422] px-6 text-sm font-bold text-[#fffaf0] shadow-lg transition-colors hover:bg-[#eb5e28] disabled:cursor-not-allowed disabled:opacity-55"
                    >
                      {loading ? (
            ...
            <AnimatePresence mode="wait">
              <motion.p
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="mx-auto mt-4 hidden max-w-sm text-center font-serif text-sm italic leading-6 text-[#77726a] xl:block"
              >
                “{QUOTES[quoteIndex]}”
              </motion.p>
            </AnimatePresence>
...
export default function LoginPage() {
  return (
    <Suspense
      fallback={
        <main className="grid h-[100svh] place-items-center overflow-hidden bg-[#fffaf0]">
          <span className="h-8 w-8 animate-spin rounded-full border-2 border-[#eb5e28] border-t-transparent" />
        </main>
      }
    >
      <LoginContent />
