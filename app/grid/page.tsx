...
const Sidebar = dynamic(() => import("@/components/Sidebar"), {
  loading: () => (
    <div className="fixed left-0 top-0 h-screen w-14 bg-[#252422] sm:w-64" />
  ),
});
  ...
  <div className="mb-8 flex flex-col gap-6 border-b border-[#252422]/10 pb-8 lg:flex-row lg:items-end lg:justify-between">
    <div>
      ...
      <p className="mt-4 max-w-xl text-sm leading-6 text-[#6d6861]">
        Every square holds one week. Look back with gratitude and make space for
        what comes next.
      </p>
    </div>
  ...
  <div className="mb-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
    {[
      ...
      >
        ...
        <p className="text-[9px] font-bold uppercase tracking-[0.15em] opacity-65">
          {stat.eyebrow}
        </p>
      ...
      <section className="mb-6 rounded-[2rem] border border-[#252422]/10 bg-white/70 p-4 shadow-[0_18px_60px_rgba(37,36,34,0.08)] sm:p-6">
        <div className="mb-5 flex flex-col gap-2 border-b border-[#252422]/10 pb-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="text-xs text-[#77726a]">
            {years.length} years · 52 weeks per row
          </p>
        </div>
        ...
        <div className="overflow-x-auto pb-3">
          <div className="flex gap-2">
            ...
            <div
              className="flex flex-shrink-0 flex-col gap-[4px]"
              onMouseLeave={() => onTooltip(null)}
            >
              {years.map((yearIndex) => (
          ...
          <div className="ml-8 mt-3 flex gap-[4px]">
            {Array.from({ length: 52 }, (_, i) => (
              <div
                key={i}
                className="w-[14px] flex-shrink-0 text-center text-[8px] font-semibold text-[#9a9287]"
              >
                {(i + 1) % 13 === 0 ? i + 1 : ""}
              </div>
            ))}
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-10">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="rounded-3xl border border-[#252422]/10 bg-white/60 px-4 py-5"
            >
...
export default function GridPage() {
  ...
  const {
    milestones,
    syncFromBackend: syncMilestones,
    getMilestone,
  } = useMilestoneStore();
  ...
  const [selectedMilestoneWeek, setSelectedMilestoneWeek] =
    useState<Week | null>(null);
  const [hydrated, setHydrated] = useState(false);
  const [highlightedWeekIndex, setHighlightedWeekIndex] = useState<
    number | null
  >(null);
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent, week: Week) => {
    e.preventDefault();
        ...
        <div className="mx-auto max-w-7xl">
          ...
          {storedDate && (
            <div className="mb-5 rounded-3xl border border-[#252422]/10 bg-white/55 p-3 shadow-sm sm:p-4">
              <Suspense
                fallback={
                  <div className="h-10 animate-pulse rounded-xl bg-[#e8e1d7]" />
                }
              >
                <DateSearch
                  birthDate={storedDate}
          ...
          <div className="mb-8 flex flex-wrap items-center gap-x-6 gap-y-3 rounded-3xl border border-[#252422]/10 bg-[#f3ede2] px-5 py-4 text-xs">
            {[
              ...
              <div key={item.label} className="flex items-center gap-2">
                <div
                  className={`h-[14px] w-[14px] rounded-[3px] ${item.icon}`}
                />
      ...
      {milestoneModalOpen && (
        <Suspense fallback={null}>
          <MilestoneModal
            ...
            existingMilestone={
              selectedMilestoneWeek
                ? getMilestone(selectedMilestoneWeek.index)
                : undefined
            }
          />
        </Suspense>
